...
function getYears(arr, callback) {
    let years = callback(arr).map(function (item) {
        return item.Year;
    }
    );
    return years;
...
function getWinners(arr, callback) {
    let winner = callback(arr).map(item => {
...
function getAverageGoals(arr, getFinalsCB) {
    ...
    return (getFinals(arr).reduce(function (acc, item) {
        return acc + item['Home Team Goals'] + item['Away Team Goals'];
    }, 0) / getFinals(arr).length).toFixed(2);
...
function getCountryWins(data, teamInitials) {
    console.log(data);
    let winNUmber = data.reduce(function (acc, current) {
        if (current['Home Team Initials'] === teamInitials && current['Home Team Goals'] > current['Away Team Goals']) {
            acc += 1;
        } else if (current['Away Team Initials'] === teamInitials && current['Away Team Goals'] > current['Home Team Goals']) {
            acc += 1;
...
console.log(getCountryWins(getFinals(fifaData), 'FRA'));
